...
class CustomDictReplayBuffer(CustomReplayBuffer):
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> CustomDictReplayBufferSamples:
        """
        Sample elements from the replay buffer.
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> CustomDictReplayBufferSamples:
        # Sample randomly the env idx
        env_indices = np.random.randint(0, high=self.n_envs, size=(len(batch_inds),))
